...
unsigned char *scaleprep(int sx, int sy, int dx, int dy)
{
 int x,y,cx,cy;
 unsigned char *retc,*coef;
 float tc1,tc2,ttc;

 retc=coef=(unsigned char *)malloc(dx*dy*4);
 ...
 for(y=dy;y>0;y--)
 {
  ...
  for(x=dx;x>0;x--)
  {
   tc1=-cx/(dx*1.0); tc2=-cy/(dy*1.0); ttc=tc1*tc2;
   coef[0]=ttc*255;       coef[1]=(tc2-ttc)*255; 
   coef[2]=(tc1-ttc)*255; coef[3]=255-coef[0]-coef[1]-coef[2];
   cx+=sx; while(cx>=0) cx-=dx;
   coef+=4;
  }
  cy+=sy; while(cy>=0) cy-=dy;
 }
 return retc;
}

/*
void Scale(unsigned char *src, unsigned char *dst, unsigned char *coef, int sx, int sy, int dx, int dy)
{
 int x,y,cx,cy;
 unsigned char *sr;
 ...
 for(y=dy;y>0;y--)
 {
  ...
  for(x=dx;x>0;x--)
  {
//   dst[0]=(coef[0]*sr[0]+coef[1]*sr[4]+coef[2]*sr[sx]+coef[3]*sr[sx+4])>>10;
//   dst[1]=(coef[0]*sr[1]+coef[1]*sr[5]+coef[2]*sr[sx+1]+coef[3]*sr[sx+5])>>10;
//   dst[2]=(coef[0]*sr[2]+coef[1]*sr[6]+coef[2]*sr[sx+2]+coef[3]*sr[sx+6])>>10;

   register int tp0,tp1,tp2;
   tp0=coef[0]*sr[0];
   tp1=coef[0]*sr[1];
   tp2=coef[0]*sr[2];
   tp0+=coef[1]*sr[4];
   ...
   tp1+=coef[3]*sr[sx+5];
   tp2+=coef[3]*sr[sx+6];
   dst[0]=tp0>>10; dst[1]=tp1>>10; dst[2]=tp2>>10;
   
   cx+=sx; while(cx>=0) { cx-=dx; sr+=4; }
   coef+=4;
   dst+=4;
  }
  cy+=sy; while(cy>=0) { cy-=dy; src+=sx*4; }
 }
}
*/
...
void Scale(unsigned char *src, unsigned char *dst, unsigned char *coef, int sx, int sy, int dx, int dy)
{
 ...
 for(y=dy;y>0;y--)
 {
  ...
  for(x=dx;x>0;x--)
  {
   dst[0]=(cf[0]*sr[0]+cf[1]*sr[1]+cf[2]*sr[sx]+cf[3]*sr[sx+1])>>8;
   cx+=sx; while(cx>=0) { cx-=dx; sr++; }
   cf+=4; dst++;
  }
  cy+=sy; while(cy>=0) { cy-=dy; src+=sx; }
 }
 sx>>=1; sy>>=1; dx>>=1; dy>>=1; cf=coef; cy=-dy;
 for(y=dy;y>0;y--)
 {
  sr=src; cx=-dx;
  for(x=dx;x>0;x--)
...
void Convert2to1(unsigned char *src, unsigned char *dst, int w, int h)
{
  unsigned int y, x;
  ...
  for (y = h; y > 0; y--) {
    ...
    for (x = w; x > 0; x--) {
      ...
      val = (srcRow0[0]+srcRow0[1]+srcRow1[0]+srcRow1[1])>>2;
      dst[0] = val;
      srcRow0 += 2; srcRow1 +=2;
      dst++;
    }
...
void ClipSrc(unsigned char *src, unsigned char *dst, int x, int y, int w, int h)
{
 int cx,cy;
 int lw=w-2*x,lh=h-2*y;
 int x2=x+x;
 ...
 {
  srcRow+=x2;
  for(cx=lw;cx>0;cx--) { *dstRow=*srcRow; dstRow++; srcRow++; }
 }
 src+=w*h; srcRow=src; 
 lw>>=1; lh>>=1; w>>=1; h>>=1; x>>=1; y>>=1; x2=w-lw;
 srcRow+=y*w-x; 
 for(cy=lh;cy>0;cy--)
 {
  srcRow+=x2;
...
PVideoInputDevice_DirectShow::~PVideoInputDevice_DirectShow()
{
    ...
    if(dFrame!=NULL) free(dFrame);
    if(coef!=NULL) free(coef);
    if(converter1!=NULL) delete converter1;
...
{
    PTRACE(1,"PVidDirectShow\tIsOpen()" << pCapture);
...
/*
 * Flip an I420 image vertically in place
 *
 */
void PVideoInputDevice_DirectShow::FlipVertical(BYTE *buffer)
{
    BYTE templine[10240];
    unsigned int psize = frameWidth*frameHeight;
    BYTE *s = buffer;
    BYTE *d = buffer + psize - frameWidth;
    unsigned int i;

    for (i=0; i<frameHeight/2; i++)
    {
	memcpy(templine, s, frameWidth);
	memcpy(s, d, frameWidth);
	memcpy(d, templine, frameWidth);
	s += frameWidth;
	d -= frameWidth;
    }

    s = buffer + psize;
    d = buffer + psize + psize/4 - frameWidth/2;
    for (i=0; i<frameHeight/4; i++)
    {
	memcpy(templine, s, frameWidth/2);
	memcpy(s, d, frameWidth/2);
	memcpy(d, templine, frameWidth/2);
	s += frameWidth/2;
	d -= frameWidth/2;
    }

    s = buffer + psize + psize/4;
    d = buffer + psize + psize/2 - frameWidth/2;
    for (i=0; i<frameHeight/4; i++)
    {
	memcpy(templine, s, frameWidth/2);
	memcpy(s, d, frameWidth/2);
...
{
  PTRACE(1, "PVidDirectShow\tGetFrameData()");
...
BOOL PVideoInputDevice_DirectShow::GetFrameDataNoDelay(BYTE *destFrame, PINDEX * bytesReturned)
{
    ...
    if(scalingMode!=0)
    {
     if(dFrame==NULL) // first call
     {
      int srcW=srcWidth-srcX*2;
      int srcH=srcHeight-srcY*2;
      PString srcMode,dstMode;
      if(converter!=NULL)
      {
       srcMode = converter->GetSrcColourFormat();
       dstMode = converter->GetDstColourFormat();
       converter1 = PColourConverter::Create(srcMode,dstMode,srcWidth,srcHeight);
       converter1->SetDstFrameSize(srcWidth,srcHeight,FALSE);
       free(tempFrame);
       frameBytes = CalculateFrameBytes(srcWidth, srcHeight, srcMode);
       tempFrame = (char *)malloc(frameBytes);
      }
      if(resizeC!=1.0)
      {
       float resC = resizeC;
       while(resC < 0.5) { resC*=2.0; srcW>>=1; srcH>>=1; }
       if(coef!=NULL) free(coef);
       coef=scaleprep(srcW,srcH,frameWidth,frameHeight);
      }
      if(dFrame!=NULL) free(dFrame);
      int fB = CalculateFrameBytes(srcWidth, srcHeight, "YUV420P");
      dFrame = (unsigned char *)malloc(fB);
     }

//     if (tempFrame == NULL) return FALSE;
     ...
     hr = pGrabber->GetCurrentBuffer(&cbBuffer, (converter1!=NULL)?(long*)tempFrame:(long*)dFrame);
     if (FAILED(hr))
     {
     ...
     if(converter1!=NULL)
     {
      converter1->Convert((BYTE *)tempFrame, dFrame, cbBuffer, bytesReturned);
     }

     if(resizeC != 1.0)
     {
      ClipSrc(dFrame,dFrame,srcX,srcY,srcWidth,srcHeight);
      int srcW=srcWidth-srcX*2, srcH=srcHeight-srcY*2; float resC=resizeC;
      while(resC < 0.5)
      {
       srcW>>=1; srcH>>=1; resC*=2.0; 
       Convert2to1(dFrame,dFrame,srcW,srcH);
      }
      Scale(dFrame,destFrame,coef,srcW,srcH,frameWidth,frameHeight);
//      Scale(dFrame,destFrame,coef,srcWidth,srcHeight,frameWidth,frameHeight);
     }
     else
     {
      ClipSrc(dFrame,destFrame,srcX,srcY,srcWidth,srcHeight);
     }
    }
...
BOOL PVideoInputDevice_DirectShow::SetFormat(const PString &wanted_format, int width, int height, int fps)
{
    ...
    float scale, bestscale=1000.0;
    unsigned int bestformat=iCount, bestXS, bestYS;
    int sFactor = 0;
    float srcSize,dstSize;
    scalingMode = 0;
	...
	if (width && (BitmapInfo->biWidth != width || BitmapInfo->biHeight != height))
	 {
	  if(width*BitmapInfo->biHeight>height*BitmapInfo->biWidth) // желаемое изображение шире, чем доступное => доступное обрезаем по вертикали
	   { srcSize = BitmapInfo->biWidth; dstSize = width; sFactor=1; }
	  else
	   { srcSize = BitmapInfo->biHeight; dstSize = height; sFactor=0; }

	  scale=(srcSize-dstSize);
	  if(dstSize > srcSize) scale/=dstSize; // изображение нужно увеличивать
	  else scale/=srcSize; // нужно уменьшать

	  if(scale>0 && scale<0.2 && (bestscale <= 0 || bestscale > scale)) 
	   { // обрезаем без масштабирования
	    bestscale=scale; bestformat=i; bestXS=BitmapInfo->biWidth; bestYS=BitmapInfo->biHeight;
	   }
	  else if(fabs(scale)<fabs(bestscale)) 
           { bestscale=scale; bestformat=i; bestXS=BitmapInfo->biWidth; bestYS=BitmapInfo->biHeight; }
          continue;
         }
	...
	/* We have match a good format, Use it to change the format */
    ...
    if(bestformat!=iCount) // define scaling mode
    {
     pStreamConfig->Release();
     if(scale>0 && scale<0.2) 
      { 
       srcWidth=bestXS; srcHeight=bestYS; 
       srcX=(bestXS-width)>>1; srcY=(bestYS-height)>>1;
      }
     else if(sFactor!=0) // обрезание по вертикали
      {
       srcWidth=bestXS; srcX=0; 
       resizeC=1.0*width/srcWidth;
       srcHeight=height*srcWidth/width; srcY=(bestYS-srcHeight)>>1;
       srcHeight=bestYS;
      }
     else
      {
       srcHeight=bestYS; srcY=0;
       resizeC=1.0*height/srcHeight;
       srcWidth=width*srcHeight/height; srcX=(bestXS-srcWidth)>>1;
       srcWidth=bestXS;
      }
     BOOL res=SetFormat(wanted_format,bestXS,bestYS,fps);
     scalingMode = 1;
     return res;
    }
...
//	return string;
    ...
    _snprintf(string, sizeof(string), "0x%8.8x", hr);
    return string;
}

...
bool H263_Base_EncoderContext::Open(CodecID codecId)
{
  ...
  _context->time_base.den = 2500;
  _context->gop_size      = 124;
...
void H263_Base_EncoderContext::SetFrameRate (unsigned fps)
{
  _context->time_base.num = 100; 
  _context->time_base.den = fps*100;
}
...
void H263_Base_EncoderContext::SetTargetBitrate (unsigned rate)
{
  ...
  else if(_width==QCIF_WIDTH) m_targetBitRate = 192*1024;
  else if(_width==CIF_WIDTH) m_targetBitRate = 384*1024;
  else if(_width==CIF4_WIDTH) m_targetBitRate = 768*1024;
  else if(_width==CIF16_WIDTH) m_targetBitRate = 1536*1024;
  ...
  */
  ...
  _context->rc_buffer_size = m_targetBitRate * 64;
}
...
// default libavcodec settings
  ...
  _context->qmax = min( _context->qmax, 24);
...
static void * create_encoder(const struct PluginCodec_Definition * codec)
{
  ...
  if (strstr(codec->destFormat, "H.263-") != NULL)
      ...
      unsigned * parmLen)
{
  ...
  if (parm != NULL) {
    ...
    if (STRCMPI(options[0], PLUGINCODEC_OPTION_MAX_BIT_RATE) == 0)
       context->SetTargetBitrate(atoi(options[1]));
    if (STRCMPI(options[0], "Frame Rate") == 0)
       context->SetFrameRate(atoi(options[1]));

//    if (STRCMPI(options[0], PLUGINCODEC_OPTION_FRAME_HEIGHT) == 0)
//       context->SetFrameHeight(atoi(options[1]));
//    if (STRCMPI(options[0], PLUGINCODEC_OPTION_FRAME_WIDTH) == 0)
//       context->SetFrameWidth(atoi(options[1]));
  }
...
{
  if (strstr(codec->sourceFormat, "H.263-") != NULL)
...
static PluginCodec_ControlDefn EncoderControls[] = {
  ...
  { PLUGINCODEC_CONTROL_SET_CODEC_OPTION,      encoder_set_option },
...
static struct PluginCodec_Option const cif4MPI =
  { PluginCodec_IntegerOption, "CIF4 MPI",  false, PluginCodec_MaxMerge, "1", "CIF4", "0", 0, "0", "4" };
...
static struct PluginCodec_Option const mediaPacketizationP =
...
static struct PluginCodec_Definition h263CodecDefn[21] = {
{ 
  PLUGIN_CODEC_VERSION_OPTIONS,       // codec API version
  &licenseInfo,                       // license information
...
{ 
  ...
  h263SQCIFDesc,                       // text decription
  YUV420PDesc,                        // source format
  h263SQCIFDesc,                       // destination format

  sqcifOptionTable,                    // user data 

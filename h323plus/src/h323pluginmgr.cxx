...
{
    public:
	cache() { seqN = FRAME_BUF_SIZE; lastN = 0; iframeN = 0; fastUpdate = 0; }
...
typedef std::map<unsigned int, cacheArray *> CacheMap;
CacheMap caches;
...
cache *CreateCacheRTP(unsigned int key)
{
 ...
 if(newCacheArray->n >= 10) return NULL;
 newCacheArray->n++;
 cout << "cache " << newCacheArray->n << "\n";
 return &(newCacheArray->c[newCacheArray->n-1]);
}

void GetFastUpdate(unsigned int key, unsigned int &flags, cache *(&srcCache))
{
...
void PutCacheRTP(unsigned int key, RTP_DataFrame &frame, unsigned int len, unsigned int flags, cache *(&dstCache))
{
...
void GetCacheRTP(unsigned int key, RTP_DataFrame &frame, unsigned int &toLen, unsigned int &seqN, unsigned int &flags, cache *(&srcCache))
{
 if(!srcCache)
 {
  CacheMap::iterator r = caches.find(key);
  if (r == caches.end()) { seqN = 0xFFFFFFFF; cout << "No cache\n"; return; }
  int i=(int)(1.0*r->second->n*(rand()/(RAND_MAX+1.0)));
  cout << "cache " << i << "\n";
  srcCache = &r->second->c[i];
 }
...
{
 while(num>=seqN) PThread::Sleep(20);
 ...
 while (r == rtpCaches.end()) { PThread::Sleep(20); r = rtpCaches.find(num); }
...
//void GetCacheRTP(unsigned int key, RTP_DataFrame &frame, unsigned int &toLen, unsigned int &seqN, unsigned int &flags);
//void PutCacheRTP(unsigned int key, RTP_DataFrame &frame, unsigned int len, unsigned int flags);
//void GetFastUpdate(unsigned int key, unsigned int &flags);
...
*/    
    ...
    if ((encoderCacheKey&0x000000FE)==0)
    {
     ...
     else if(frameWidth==QCIF_WIDTH) encoderCacheKey|=8;
...
// ìîæíî âûïîëíÿòü äî îòêðûòèÿ ïîòîêà
void H323PluginVideoCodec::SetVideoSize(int _width, int _height)
{
    ...
    if (_width*_height > frameWidth*frameHeight) return;
         ...
         (
           ((encoder.flags & PluginCodec_MediaTypeMask) == PluginCodec_MediaTypeAudio) && 
            (strcmp(encoder.sourceFormat, "L16") == 0 ||
             strstr(encoder.sourceFormat, "PCM") == encoder.sourceFormat)
         ) ||
         (

...
bool FFMPEGLibrary::Load()
{
  ...
  if (!DynaLink::Open("avcodec.dll")
#if defined(WIN32)
      && !DynaLink::Open("libavcodec.dll")
#else
...
void H263EncoderContext::SetTargetBitrate (unsigned rate)
{
  bitRate = rate;
  if(frameWidth==SQCIF_WIDTH) bitRate = 96*1024;
  else if(frameWidth==QCIF_WIDTH) bitRate = 192*1024;
  else if(frameWidth==CIF_WIDTH) bitRate = 384*1024;
  else if(frameWidth==CIF4_WIDTH) bitRate = 768*1024;
  else if(frameWidth==CIF16_WIDTH) bitRate = 1536*1024;
  if(rate < bitRate) bitRate = rate;
      ...
      unsigned * parmLen)
{
  ...
  if (parm != NULL) {
    ...
    if (STRCMPI(options[0], "Max Bit Rate") == 0)
...
static PluginCodec_ControlDefn EncoderControls[] = {
  ...
  { "set_codec_option", encoder_set_option },
  { NULL }
};
...
static struct PluginCodec_Option const qcifMPI =
  { PluginCodec_IntegerOption, "QCIF MPI", false, PluginCodec_MaxMerge, "1", "QCIF", "0", 0, "0", "6" };

static struct PluginCodec_Option const cifMPI =
  { PluginCodec_IntegerOption, "CIF MPI",  false, PluginCodec_MaxMerge, "1", "CIF",  "0", 0, "0", "6" };

static struct PluginCodec_Option const sqcifMPI =
  { PluginCodec_IntegerOption, "SQCIF MPI", false, PluginCodec_MaxMerge, "1", "SQCIF", "0", 0, "0", "6" };

static struct PluginCodec_Option const cif4MPI =
  { PluginCodec_IntegerOption, "CIF4 MPI",  false, PluginCodec_MaxMerge, "1", "CIF4", "0", 0, "0", "6" };
